...
def original_skip_tts():
    """Save and restore original YAKK_SKIP_TTS value"""
    original = os.environ.get("YAKK_SKIP_TTS")
    yield
    if original is None:
        os.environ.pop("YAKK_SKIP_TTS", None)
    else:
        os.environ["YAKK_SKIP_TTS"] = original
...
class TestSkipTTS:
    ...
    async def test_skip_tts_true_overrides_env(self, voice_mode_server, original_skip_tts):
        ...
        os.environ["YAKK_SKIP_TTS"] = "false"
