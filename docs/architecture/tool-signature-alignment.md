# Tool Signature Alignment: Yakk Local vs Yakk Connect
...
Align the tool names and function signatures between Yakk (local MCP) and Yakk Connect (cloud MCP) so that:
...
3. How do we handle the case where an agent has both local Yakk and Yakk Connect installed?
